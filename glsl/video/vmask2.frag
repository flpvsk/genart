...
void main (void) {
    ...
    vec4 sample = vec4(chromaAB(u_tex0, st, vec2(0.5, 1.), vec3(0.05*fn1(u_time), 0.01, 0.02)), 1.);
    ...
    float y1 = 0.2;
    float y2 = 0.5;
    float shape = mix(circle(st, 0.3), circle(st, 0.6), fn1(0.1 * u_time));
    float isFrame = shape * (
      step(1. - y2, st.y)
      * (1. - step(1. - y1, st.y))
    );
    ...
    color = sample * isNotFrame + vec4(mouseSample) * isFrame;
    vec3 c = color.rgb;
    // c = decimation(c, 20.);
    // c += circle(st, .5);
    color = vec4(c * color.rgb, 1.);
#endif
