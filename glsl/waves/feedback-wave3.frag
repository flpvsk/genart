...
void main() {
    ...
    // st.x *= aspect;
    float dotSize = max(2. / u_resolution.x, 2. / u_resolution.y);
    float dotSmooth = 0.2 * dotSize;
    float phase = u_speed * u_time;
    ...
    vec3 prev = texture2D(u_buffer3, vec2(st.x + 0.01 * u_speed, st.y)).rgb;
